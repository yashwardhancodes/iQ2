...
export default function DashboardLayout({ children }) {
    ...
    return (
        <div className="flex h-screen bg-white text-black">
            {/* Sidebar */}
            <div
                className={`fixed md:relative inset-y-0 left-0 bg-white h-screen overflow-y-auto transition-transform duration-300 z-40 p-4 shadow-xl font-poppins w-80 ${isOpen ? "translate-x-0" : "-translate-x-full"
                    } md:translate-x-0`}
            >
                {/* Close button (Mobile) */}
                    ...
                    {menuItems.map((item) => {
                        ...
                        return (
                            <Link
                                key={item.path}
                                href={item.path}
                                className={`flex items-center gap-3 p-2 rounded-full transition-colors ${isActive ? " text-black  " : "hover:bg-orange-100 text-black"
                                    }`}
                            >
                                <div
                                    className={`p-3 rounded-full ${isActive ? "bg-[rgba(239,156,1,1)] text-white    " : "bg-[rgba(116,205,255,0.15)] p-3 text-blue-800"
                                        }`}
                                >
                                    {item.icon}
                                </div>
